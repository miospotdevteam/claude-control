...
This skill does not replace engineering-discipline — it adds **structure** (plan files, sub-plans, the execution loop) on top of the **behavior** (thoroughness, blast radius checks, verification) that engineering-discipline provides.
...
Do NOT rely on telling the user to `/compact` — auto-compaction will fire before you get the chance. Instead, save progress continuously so that compaction at any moment loses zero meaningful state.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/persistent-plans/scripts/init-plan-dir.sh
```

This creates the directory structure and a `.gitignore` inside `.temp/` so plan files are never accidentally committed.
...
The masterPlan format is documented in `references/master-plan-format.md`. Read that file for the exact template. The critical sections are:

- **Context**: What the user asked for and key constraints
- **Discovery Summary**: Everything you learned during exploration — this is your gift to your future compacted self. Include file paths, patterns found, dependencies, conventions. Write enough that a fresh context window does not need to re-explore the codebase.
...
- It involves a **repetitive sweep** across many files (e.g., "replace all silent catches", "update all API calls", "add i18n keys everywhere")
...
See `references/sub-plan-format.md` for the template.
...
Think of it like autosave in a video game — save constantly so that when the crash (auto-compaction) happens, you lose minimal progress.
...
When you complete a step, the Result field should capture what you actually did:
- Files created/modified
- Decisions made and why
- Anything unexpected that the next context window needs to know

Bad: `"Done."`
Good: `"Created apiClient.ts in src/lib/ with typed wrappers for all 5 endpoints. Used the existing AuthContext for token injection rather than a separate interceptor — matches the pattern in src/lib/analytics.ts. Updated imports in 3 consumer files: Dashboard.tsx, Settings.tsx, Profile.tsx."`
...
2. Find the most recently modified masterPlan.md in `active/` (or use the one the hook told you about)
...
5. If the step has a **Progress** checklist, check which items are done — that tells you exactly where within the step to resume
...
**You MUST do this before touching any code.** The plan on disk is the source of truth, not your memory of what you were doing.

### If an in-progress step exists

A step marked `[~] in-progress` means compaction happened mid-step. Read the step's Progress checklist and any partial Result notes. The checked-off Progress items tell you what's been done. Assess the state (check git status, look at files) and continue from where the Progress checklist left off.
...
- **Never delete a plan** — when all steps are complete, move the plan folder from `active/` to `completed/`
- **If requirements change** — update masterPlan FIRST, then continue execution
- **The Discovery Summary is sacred** — write it thoroughly during exploration; your compacted future self will thank you
- **Use the scripts** — run `plan-status.sh` to see all plan states, run `resume.sh` to find what to pick up next

### Script usage

Check plan status across all active plans:
```bash
bash .temp/plan-mode/scripts/plan-status.sh
```

Find what to resume after compaction:
```bash
bash .temp/plan-mode/scripts/resume.sh
```
...
- `references/master-plan-format.md` — exact masterPlan.md template with all fields including Progress
- `references/sub-plan-format.md` — exact sub-plan template with Groups for sweep-type work
- `references/claude-md-snippet.md` — recommended additions to project CLAUDE.md

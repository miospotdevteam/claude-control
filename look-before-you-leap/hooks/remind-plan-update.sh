...
output = {
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            ...
            "1. Update progress via plan_utils.py (update-step, update-progress, set-result)\n"
            "2. Mark completed progress items as done\n"
            "3. Add notes to the current step's result field\n\n"
            "plan_utils.py writes to progress.json (mutable state). "
            "plan.json is immutable after approval — do NOT edit it directly.\n\n"
            "If auto-compaction fired RIGHT NOW, could you resume from plan.json + progress.json alone? "
